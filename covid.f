	...
	real*8 dtau,taup,ap,gnorm,xt,t_rand(10000000),gp,g
	character stringa*32
...
c***** INPUT PARAMETERS **********
	...
	ap=6.2/taup 
	ndayt=60d0/taup
	ninf=250 !number I(0) of infected persons on day 0
	g=0d0
	dtau=taup/100000d0
	gnorm=1d0/(gamma(ap)*taup**ap)
	i=0

c** computing random time according to the probability generation function w

	do j=1,int(taup/dtau)*ndayt
	   xt=j*dtau
	   g=g+dtau*(xt**(ap-1d0)*exp(-xt*1d0/taup))
	   if(g*gnorm.ge.gp+1./6000)then
	      i=i+1
	      t_rand(i)=xt
	      gp=g*gnorm
	     ! print*,i
...
c*********** ZERO-TH ORDER GENERATION **********
c** from the daily number of imported infections xnimm, we evaluate the
c** number of infected people

	open(88,file='rt2.dat',status='old')
	read(88,*)stringa    !reading the first line of the file
	do i=1,1000000
	   itime(i)=-1
	enddo
	nevent=0
	do i=1,1000000
	   ...
	   xnimm=xnimm  
	   do k=nevent+1,nevent+int(xnimm)
	      itime(k)=i
	   enddo
	   nit(i)=int(xnimm)
	   nevent=nevent+int(xnimm)
	   write(56,*)j,bra(i)
	enddo
 99	jfinal=i-1

	seed1=-39814113  !seed for random number

	do j=1,ninf  !loop on the number of infected persons at day 0
	   nevent=nevent+1
	   time=1
	   itime(nevent)=time
	   nit(int(time))=nit(int(time))+1
	enddo
	
c***** FIRST ORDER GENERATION ************
c***** each "mother" infector generates a number of new infected elements.
c***** The number of off-springs depends on Rs at the time of their mother,
c***** according to a Poisson distribution  

	do j=1,100000000
	   tt0=itime(j)  !occurrence time of their mother
	   muaat=Bra(int(tt0)) !expected value of infected of off-springs
	   nn=ZBQLPOI(muaat) !evaluation from Poisson distribution
	   
	   if(itime(j).lt.0)exit
	   do i=1,nn
	      rr=ran2(seed1)
	      time=tt0+t_rand(int(rr*ifinal))+.5 !random infected time from w
	      if(time.le.jfinal)then
		 nevent=nevent+1
		 itime(nevent)=time
		 nit(int(time))=nit(int(time))+1
	      endif
	...
	do j=1,jfinal-1
	   write(48,*)j+55,nit(j)*(1+ntest(j)*flag_test),nit(j)
	   !the presence of +55 is justified because it is considered the 
           !first case in Italy on the 55th day of the year
	enddo	
